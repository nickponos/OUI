...
import {saveUserProfile} from '../../model/actions/signupAC';
...
class CompleteYourProfile extends Component {
    ...
    _addPhoto() {
        ...
        //     //includeBase64: true
        // }).then(image => {
        //     // console.log("Cropped Image URL === ", image);
    ...
    _completeProfile() {
        const ext = this.state.profile.split('.').pop(); // Extract image extension
        const filename = `${Date.now()}.${ext}`; // Generate unique name
        this.setState({ loading: true });
        storage
        .ref(`images/${filename}`)
        .putFile(this.state.profile)
        .then((res) => {
            let user = {
                profile: res.downloadURL,
            ...
            navigationService.navigate(screens.GENDER);
        }).catch((error) => {
...
export default connect(mapStateToProps)(CompleteYourProfile);

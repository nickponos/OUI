...
import { firestore, db, storage }  from '../../../components/Firebase';
...
export default class EditProfile extends Component {
    ...
    openPicker = (index) => {
        ImagePicker.openPicker({
            ...
            cropping: true
              ...
              const {aboutImages } = this.state;
              aboutImages[index] = image.path
              this.setState({aboutImages})
          });
    }
    ...
    _editPhoto = (value, index) => {
        ...
        ActionSheet.showActionSheetWithOptions({
            ...
            (buttonIndex) => {
                switch (buttonIndex) {
                    case 0:
                        this._addPhoto('edit', index)//this.openPicker(index)
                        break
                    case 1:
    ...
    _inputBox(name, onChange, value, mline, ) {
        return (
            <View style={styles.textinputcontainer}>
                <View style={{ width: '30%', }}>
                    <Text style={{ fontSize: RFValue(13.6), fontFamily: Platform.OS == 'ios' ? 'Lato-Bold' : 'Lato Bold', color: 'black' }}>{name}</Text>
                </View>
                <View style={{ width: '70%', }}>
                    <TextInput
                        style={styles.text}
    ...
    _about() {
        ...
        return (
            <View style={{ width: '100%', borderBottomWidth: 1, borderColor: '#f2f3f5', marginTop: 20, paddingVertical: 10 }}>
                <Text style={{ paddingHorizontal: 20, fontSize: RFValue(19.04), fontFamily: Platform.OS == 'ios' ? 'Lato-Bold' : 'Lato Bold', color: 'black' }}>About</Text>
                    ...
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} bounces={false}>
                        <FlatList
                            ...
                            renderItem={({ item, index }) =>
                                <TouchableOpacity onPress={() => { this._editPhoto('edit', index) }} style={{ alignItems: 'center', marginRight: 15 }}>
                                    <View style={styles.addImageContainer}>
                                        <Image source={{ uri: item.image }} style={{ height: 60, width: 60, borderRadius: 15 }} />
                                    </View>
                                    <Text style={{ color: "#9c53e4", fontSize: RFValue(13.6), marginVertical: 8 }}>Edit/Delete</Text>
                                </TouchableOpacity>
                            }
                        ...
                        {this.state.aboutImages.length >= 3 ? null
                            ...
                            <TouchableOpacity onPress={() => this._addPhoto('about', this.state.aboutImages.length)} style={{ alignItems: 'center', marginRight: 15, }}>
                                ...
                                <Text style={{ color: "#9c53e4", fontSize: RFValue(13.6), marginVertical: 8 }}>Add</Text>
                            </TouchableOpacity>
    ...
    searchFor() {
        return (
            <View style={{ width: '100%', marginTop: 20, paddingVertical: 10 }}>
                <Text style={{ paddingHorizontal: 20, paddingBottom: 10, fontSize: RFValue(19.04), fontFamily: Platform.OS == 'ios' ? 'Lato-Bold' : 'Lato Bold', color: 'black' }}>I'm searching for</Text>
    ...
    _uploadImage(image, userProfile, photo) {
        ...
        return storage
            ...
            .putFile(image).then(res => {
                if (userProfile) {
                    // user = db.ref().child('Users/' + userId).update({ profile: res.downloadURL })
                    user = firestore.collection('newUsers').doc(userId).update({ profile: res.downloadURL });
                }
                else {
                    if (photo == 1) {
                        // user = db.ref().child('Users/' + userId).update({ aboutImages1: res.downloadURL })
                        user = firestore.collection('newUsers').doc(userId).update({ aboutImages1: res.downloadURL });
                        this.setState(state => (this.state.aboutImages[0].image = res.downloadURL, state))
                    }
                    if (photo == 2) {
                        // user = db.ref().child('Users/' + userId).update({ aboutImages2: res.downloadURL })
                        user = firestore.collection('newUsers').doc(userId).update({ aboutImages2: res.downloadURL });
                        this.setState(state => (this.state.aboutImages[1].image = res.downloadURL, state))
                    }
                    if (photo == 3) {
                        // user = db.ref().child('Users/' + userId).update({ aboutImages3: res.downloadURL })
                        user = firestore.collection('newUsers').doc(userId).update({ aboutImages3: res.downloadURL });
    ...
    _done() {
        ...
        // user = db.ref().child('Users/' + userId).update({
        //     bio: bio,
        //     birthday: birthday,
        //     city: location,
        //     name: name,
        ...
        user = firestore.collection('newUsers').doc(userId).update({
            bio: bio,
            birthday: birthday,
            city: location,
            name: name,
        ...
        if (aboutImages.length > 1) {
            ...
            if (this.state.aboutImages.length > 2)
                ...
                <SafeAreaView style={styles.container}>
                    ...
                    <GooglePlacesAutocomplete
                        ...
                        renderRightButton={() => <TouchableOpacity style={{ width: '20%', justifyContent: 'center', alignItems: 'center' }} onPress={() => this.setState({ search: false })}>
                            <Text style={{ fontSize: RFValue(13.6) }}>Cancel</Text>
                        </TouchableOpacity>}
                    /></SafeAreaView>)
        }
        else {
                ...
                <SafeAreaView style={styles.container}>
                    ...
                    <View style={styles.headerView}>
                        <TouchableOpacity style={styles.backView}
                            ...
                            <Text style={{ color: '#9246e6', fontSize: RFValue(14.96), fontFamily: Platform.OS == 'ios' ? 'Lato-Regular' : 'Lato Regular' }}>Cancel</Text>
                        </TouchableOpacity>
                        <View style={{ width: '70%', alignItems: 'center', paddingRight: '10%' }}>
                            <Text style={{ fontSize: RFValue(14.96), fontFamily: Platform.OS == 'ios' ? 'Lato-Bold' : 'Lato Bold', color: '#000' }}>Edit Profile</Text>
                        </View>
                        <TouchableOpacity style={[styles.backView, { width: '150%' }]}
                            onPress={() => this._done()}>
                            <Text style={{ color: 'grey', fontSize: RFValue(14.96), fontFamily: Platform.OS == 'ios' ? 'Lato-Regular' : 'Lato Regular' }}>Done</Text>
                        </TouchableOpacity>
                    </View>
                    ...
                    <ScrollView contentContainerStyle={{ paddingBottom: '5%' }}>
                        ...
                        <View style={{ width: '100%', alignItems: 'center', borderBottomWidth: 1, borderTopWidth: 1, borderColor: '#f2f3f5', paddingVertical: 18 }}>
                            <TouchableOpacity onPress={() => this._addPhoto('profile')} style={{ alignItems: 'center', width: '30%' }}>
                                ...
                                <Text style={{ color: "#9c53e4", fontSize: RFValue(13.6), fontFamily: Platform.OS == 'ios' ? 'Lato-Regular' : 'Lato Regular', marginVertical: 8 }}>Edit</Text>
                            </TouchableOpacity>
                        </View>
                        ...
                        <View style={[styles.textinputcontainer, { paddingVertical: 15 }]}>
                            <View style={{ width: '30%' }}>
                                <Text style={{ fontSize: RFValue(13.6), fontFamily: Platform.OS == 'ios' ? 'Lato-Bold' : 'Lato Bold', color: 'black' }}>Birthday</Text>
                            </View>
                            <View>
                        ...
                        <View style={styles.textinputcontainer}>
                            <View style={{ width: '30%' }}>
                                <Text style={{ fontSize: RFValue(13.6), fontFamily: Platform.OS == 'ios' ? 'Lato-Bold' : 'Lato Bold', color: 'black' }}>Location</Text>
                            </View>
                            <View>
                                <TextInput
                                    style={styles.text}

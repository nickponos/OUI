...
export default class InviteGuest extends Component {
        ...
        if (this.state.going.length > 0) {
            ...
            return (
                <View style={{ alignSelf: 'center', marginTop: '30%', flex: 1, width: '95%' }}>
                    <Text style={{ paddingBottom: 10, fontSize: RFValue(18.36), color: '#000', fontFamily: Platform.OS == 'ios' ? 'Lato-Bold' : 'Lato Bold', textAlign: 'center' }}>No guests yet</Text>
                    <Text style={{ fontSize: RFValue(17), color: '#d8d7dd', fontFamily: Platform.OS == 'ios' ? 'Lato-Regular' : 'Lato Regular', textAlign: 'center' }}>Invite people to participate in your event </Text>
                </View>
            )
        }
    }
    ...
    _pending() {
        ...
        if (this.state.pending.length > 0) {
                                    ...
                                    <View style={{flexDirection:'row',alignItems:'center'}}>
                                        <TouchableOpacity style={[styles.buttoncontainer]}>
                                            <LinearGradient start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} colors={['#a65ae1', '#8a4cea']} style={{ flexDirection: 'row', width: '100%', borderRadius: 25, alignItems: 'center', height: 35, justifyContent: 'center' }}>
                                                ...
                                                <Text style={{ color: '#fff', fontSize: RFValue(15.64), fontFamily: Platform.OS == 'ios' ? 'Lato-Regular' : 'Lato Regular' }}>Add</Text>
                                            </LinearGradient>
                                        </TouchableOpacity>
            ...
            return (
                <View style={{ alignSelf: 'center', marginTop: '30%', flex: 1, width: '95%' }}>
                    <Text style={{ paddingBottom: 10, fontSize: RFValue(18.36), color: '#000', fontFamily: Platform.OS == 'ios' ? 'Lato-Bold' : 'Lato Bold', textAlign: 'center' }}>No pending invition yet</Text>
                    <Text style={{ fontSize: RFValue(17), color: '#d8d7dd', fontFamily: Platform.OS == 'ios' ? 'Lato-Regular' : 'Lato Regular', textAlign: 'center' }}>Invite people to participate in your event </Text>
                </View>
            )
        }
    ...
    _invited() {
        ...
        if (this.state.invited.length > 0) {
                    ...
                    <FlatList
                     showsVerticalScrollIndicator={false}
                        ...
                        <View style={{width:'90%',alignSelf:'center',flexDirection:'row',justifyContent:'space-between',alignItems:'center'}}>
                            ...
                            <TouchableOpacity style={[styles.buttoncontainer]}>
                                     <LinearGradient start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} colors={['#f5f6f8', '#f5f6f8']} style={{ flexDirection: 'row', width: '100%', borderRadius: 25, alignItems: 'center', height: 30, justifyContent: 'center' }}>
                                         <Text style={{ color: '#000', fontSize: RFValue(15.64), fontFamily: Platform.OS == 'ios' ? 'Lato-Regular' : 'Lato Regular' }}>Invited</Text>
                                     </LinearGradient>
                                 </TouchableOpacity>
                        </View>
                    </View>
            ...
            return (
                <View style={{ alignSelf: 'center', marginTop: '30%', flex: 1, width: '95%' }}>
                    <Text style={{ paddingBottom: 10, fontSize: RFValue(18.36), color: '#000', fontFamily: Platform.OS == 'ios' ? 'Lato-Bold' : 'Lato Bold', textAlign: 'center' }}>No guests invited yet</Text>
                    <Text style={{ fontSize: RFValue(17), color: '#d8d7dd', fontFamily: Platform.OS == 'ios' ? 'Lato-Regular' : 'Lato Regular', textAlign: 'center' }}>Invite people to participate in your event </Text>
                </View>
            )
        }
                ...
                <View style={styles.subContainerView}>
                    <View style={{ borderBottomWidth: 1, borderColor: '#f2f3f5' }}>
                        <View style={{ width: '90%', paddingVertical: 5, flexDirection: 'row', alignItems: 'center', alignSelf: 'center', justifyContent: 'space-between' }}>
                            ...
                            <TouchableOpacity style={{ width: 50, alignItems: 'flex-end' }} onPress={() => {navigationService.navigate(screens.SEARCH_GUEST)}}>
                            <Icon name='md-add' type='Ionicons' style={{ color: '#9246e6', fontSize: 25 }} />
                            </TouchableOpacity>
                        </View>
                        <View style={{ marginTop: 10, marginBottom: 7, marginHorizontal: 20 }}>
                        <Text style={{ fontSize: RFValue(25.84), fontFamily: Platform.OS == 'ios' ? 'Lato-Bold' : 'Lato Bold', color: '#000' }}>Guests</Text>
                    </View>
